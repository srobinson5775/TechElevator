...
SELECT * FROM state WHERE state_abbreviation ='CA'; --double check - always run where clause with a select statement first before update to veryify what you want to update
...
SELECT * FROM city; --how you know to use AND, possible duplicate states want to specify by using WHERE and also AND
...
SELECT * FROM city WHERE city_name = 'Hawkins' AND state_abbreviation = 'IN'; -- always run to verify what is being deleted

DELETE FROM city WHERE city_name = 'Hawkins' AND state_abbreviation ='IN';

-- Delete all cities with a population of less than 1,000 people from the city table.
SELECT * FROM city WHERE population < 1000; -- double check before deleting by using a SELECT statement
...
INSERT INTO city (city_name, state_abbreviation, population, area) -- what we are adding
...
UPDATE state
SET state_nickname = 'Vacationaland'
WHERE state_abbreviation ='CA';
