...
export default {
  ...
  methods: {
    ...
    deleteBoard() {
      if (confirm("Are you sure you want to delete this board and ALL associated cards? This can not be undone."))
      {
        ...
        .catch(error => {
            if(error.response){
              this.errorMsg = "Error deleting new board. Response recieved was " + error.response.status

            } else if (error.request) {
              this.errorMsg = "Error deleting new board. Server could not be reached.";
            } else{
